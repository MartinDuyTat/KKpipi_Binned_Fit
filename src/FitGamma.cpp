...
void FitGamma::PlotContours(std::string Filename_rB_deltaB, std::string Filename_deltaB_gamma, std::string Filename_gamma_rB, unsigned int npoints) const {
  ...
  TCanvas *c1 = new TCanvas("rB_vs_dB", "r_B vs #delta_B contours", 200, 10, 600, 400);
  ...
  gr1->GetXaxis()->SetTitle("r_B");
  gr1->GetYaxis()->SetTitle("#delta_B");
  c1->SaveAs(Filename_rB_deltaB.c_str());
  delete c1;
  delete gr1;
  TCanvas *c2 = new TCanvas("deltaB_vs_gamma", "#delta_B vs #gamma contours", 200, 10, 600, 400);
  m_minimiser->SetErrorDef(1.0);
  m_minimiser->Contour(1, 2, npoints, x.data(), y.data());
  x[npoints] = x[0];
  y[npoints] = y[0];
  TGraph *gr4 = new TGraph(npoints + 1, x.data(), y.data());
  gr4->Draw("AC");
  gr4->SetTitle("#delta_B vs #gamma contours");
  gr4->GetXaxis()->SetTitle("#delta_B");
  gr4->GetYaxis()->SetTitle("#gamma");
  c2->SaveAs(Filename_deltaB_gamma.c_str());
  delete c2;
  delete gr4;
  TCanvas *c3 = new TCanvas("gamma_vs_rB", "#gamma vs r_B contours", 200, 10, 600, 400);
  m_minimiser->SetErrorDef(1.0);
  ...
  gr7->Draw("AC");
  gr7->SetTitle("#gamma vs r_B contours");
  gr7->GetXaxis()->SetTitle("#gamma");
  gr7->GetYaxis()->SetTitle("r_B");
