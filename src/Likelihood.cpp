...
double Likelihood::operator()(const double *cpparameters) {
  ...
  std::vector<double> predictedBplus, std::vector<double> predictedBminus;
  ...
  m_bins.Predict(m_ddparameters, cpparam, eventsBplus, eventsBminus, totalBplus, totalBminus);
  for(int i = 0; i < m_bins.NumberBins(); i++) {
