...
int RectangularPhaseSpace::WhichBin(const Event &event) const {
  ...
  int number = m_BinMap[0].lower_bound(X[0])->second + m_BinMap[1].lower_bound(X[1])->second*m_Binning[0] + m_BinMap[2].lower_bound(X[2])->second*m_Binning[0]*m_Binning[1] + m_BinMap[3].lower_bound(X[3])->second*m_Binning[0]*m_Binning[1]*m_Binning[2] + m_BinMap[4].lower_bound(X[4])->second*m_Binning[0]*m_Binning[1]*m_Binning[2]*m_Binning[3];
  return number;
}
